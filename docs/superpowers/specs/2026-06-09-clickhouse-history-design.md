...
- **Container runtime (podman):** Testcontainers works with podman via its Docker-API socket. The expected dev/CI setup enables the rootless socket and points Testcontainers at it:
  ```bash
  systemctl --user enable --now podman.socket
  export DOCKER_HOST="unix:///run/user/$(id -u)/podman/podman.sock"
  export TESTCONTAINERS_RYUK_DISABLED=true   # Ryuk reaper is flaky rootless; JVM shutdown hook cleans up instead
  ```
  These steps go in the test runbook (`docs/` or README). With the socket up, `HistoryIntegrationTest` runs normally.
- **Gating as a fallback only:** the test is guarded by `assumeTrue(dockerAvailable())` so a machine *without* a reachable socket **skips** it rather than failing `./gradlew build`. This is a safety net for environments that haven't enabled the socket — not the intended default; the socket-enabled path is.
